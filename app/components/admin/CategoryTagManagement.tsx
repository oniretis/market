...
export function CategoryTagManagement() {
  ...
  useEffect(() => {
    ...
    const connectSSE = () => {
      const eventSource = new EventSource("/api/admin/categories/stream");

      eventSource.onopen = () => {
        console.log('Categories SSE connection established');
      };
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (data.error) {
            console.error('Categories SSE Error:', data.error);
            return;
          }
          if (data.build) {
          ...
          console.log('Categories and tags updated via SSE');
        } catch (error) {
          console.error('Error parsing categories SSE data:', error);
        }
      };

      eventSource.onerror = (error) => {
        console.error('Categories SSE connection error, falling back to polling', error);
        eventSource.close();
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        console.log('Tab became visible, reconnecting categories SSE...');
        connectSSE();
      }
    };
