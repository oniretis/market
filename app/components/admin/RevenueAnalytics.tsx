...
export function RevenueAnalytics() {
  ...
  useEffect(() => {
    ...
    const connectSSE = () => {
      const eventSource = new EventSource("/api/admin/analytics/revenue/stream");

      eventSource.onopen = () => {
        console.log('Revenue SSE connection established');
      };
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (data.error) {
            console.error('Revenue SSE Error:', data.error);
            return;
          }
          if (data.build) {
          ...
          setRevenueData(data);
          console.log('Revenue data updated via SSE');
        } catch (error) {
          console.error('Error parsing revenue SSE data:', error);
        }
      };

      eventSource.onerror = (error) => {
        console.error('Revenue SSE connection error, falling back to polling', error);
        eventSource.close();

        // Fallback to polling if SSE fails (5 minutes for analytics)
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        console.log('Tab became visible, reconnecting revenue SSE...');
        connectSSE();
      }
    };
