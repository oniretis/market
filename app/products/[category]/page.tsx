...
import { createCategoryMetadata, generateJSONLD, createBreadcrumbStructuredData } from "@/app/lib/seo";
...
async function getData(category: string) {
  ...
  return { data, categoryRecord };
}

// Generate metadata for the category
export async function generateMetadata({ params }: { params: { category: string } }): Promise<Metadata> {
  const { data, categoryRecord } = await getData(params.category);
  ...
  if (!categoryRecord && params.category !== "all") {
    return {
...
export default async function CategoryPage({
  ...
  const breadcrumbStructuredData = createBreadcrumbStructuredData([
    ...
    { name: title, url: `/products/${params.category}` },
  ]);
